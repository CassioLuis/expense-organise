import { GoalCard } from './goal-card'
import { useGoalStore } from '@/infra/store/goal-store'
import { categoryStore } from '@/infra/store/category-store'
import { useAppDependencies } from '@/hooks/use-app-dependencies'
import Utilities from '@/utils/Utilities'

export function GoalsGrid () {
  const goals = useGoalStore((state) => state.goals)
  const storeUpdateGoal = useGoalStore((state) => state.storeUpdateGoal)
  const storeSetGoals = useGoalStore((state) => state.storeSetGoals)
  const { upsertGoalUsecase } = useAppDependencies()

  const categories = categoryStore((state) => state.categories)

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mt-6 pb-24">
      {categories.map((cat, idx) => {
        const existing = goals.find(g => g.categoryName === cat.name)
        const currentAmount = existing ? existing.amount : 0
        const maxAllocable = 5000

        return (
          <GoalCard
            key={idx}
            color={Utilities.stringToColor(cat.name)}
            title={cat.name}
            maxAllocable={maxAllocable}
            currentAmount={currentAmount}
            onChange={(val) => storeUpdateGoal(cat.name, val)}
            onSave={() => upsertGoalUsecase.execute(goals, storeSetGoals)}
          />
        )
      })}
    </div>
  )
}
